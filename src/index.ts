#!/usr/bin/env node
/**
 * Drata MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own Drata API key as Bearer token.
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Drata GRC — controls, evidence, policies, risks, vendors, frameworks, personnel, monitoring tests — through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Drata API key as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-drata-api-key>',
      howToGetKey:
        'Generate an API key in Drata at Settings → API Keys',
    },
    configTemplate: {
      mcpServers: {
        drata: {
          ...
          headers: {
            Authorization: 'Bearer <your-drata-api-key>',
          },
        },
      },
    },
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/drata/',
    },
  });
});
...
<head>
  ...
  <title>Drata MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Drata MCP Server</h1>
    <p class="subtitle">Access Drata GRC — controls, evidence, policies, risks, vendors, frameworks, personnel, and monitoring tests — through ${tools.length} MCP tools.</p>

    <div class="card">
      <h2>How to get your Drata API key</h2>
      <ol class="steps">
        <li>Log in to <a href="https://app.drata.com" target="_blank" rel="noopener">Drata</a>.</li>
        <li>Go to <strong>Settings → API Keys</strong>.</li>
        <li>Click <strong>Create API Key</strong> and select the scopes you need.</li>
    ...
    <div class="card">
      ...
      <label for="apiKey">Drata API Key</label>
...
function createMCPServer(client: DrataClient): Server {
  const server = new Server(
    { name: 'drata-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
<head>
  ...
  <title>Authorize \u2014 Drata MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Drata MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Drata MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Drata MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (client provides Drata API key)`);
});
